...
def schedule_email_report(  # pylint: disable=unused-argument
    ...
    logging.info("================received celery schedule========================")
...
def deliver_alert(alert: Alert) -> None:
    ...
    img_data = None
    images = {}
    logging.info("Triggering alert.slice: %s", alert.slice)
    ...
    if alert.slice:
        ...
        # )
        slc = db.session.query(Slice).filter_by(id=alert.slice.id).one()
        report_content = _get_slice_visualization(slc, EmailDeliveryType.inline)
        _deliver_email(
            alert.recipients,
            deliver_as_group,
            subject,
...
def schedule_window(
    ...
    for schedule in schedules:
        ...
        logger.info("============last_eval_dttm: %s ======start_at: %s========", schedule.last_eval_dttm, start_at)
        if (
            hasattr(schedule, "last_eval_dttm")
            and schedule.last_eval_dttm
            and pytz.utc.localize(schedule.last_eval_dttm) > start_at
        ):
            schedule_start_at =  pytz.utc.localize(schedule.last_eval_dttm) + timedelta(seconds=1)
