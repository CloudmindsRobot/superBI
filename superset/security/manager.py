...
class RolesFilter(BaseFilter):
    def apply(self, query, value):
       # query = query.join(User)
        ...
        query = query.join(assoc_user_role,  assoc_user_role.c.role_id == Role.id).join(User, User.id == assoc_user_role.c.user_id)
        value = set_value_to_type(self.datamodel, self.column_name, value)
        return query.filter(User.id == g.user.id)

class RolePermissionsFilter(BaseFilter):
    def apply(self, query, value):
        user_roles = [role.name.lower() for role in g.user.roles]
        ...
        if "admin" in user_roles:
            return query
        query, field = get_field_setup_query(query, self.model, self.column_name)
        query = query.join(assoc_permissionview_role, assoc_permissionview_role.c.permission_view_id == field).join(Role, Role.id == assoc_permissionview_role.c.role_id)
        user_ids = [role.id for role in g.user.roles]
        return query.filter(Role.id.in_(user_ids))
...
RoleModelView.base_filters = [['permissions', RolesFilter, 'm']]
RoleModelView.related_views = [UserModelView, PermissionViewModelView]
RoleModelView.edit_form_query_rel_fields = {'permissions': [['id',RolePermissionsFilter,0]]}
...
class SupersetSecurityManager(  # pylint: disable=too-many-public-methods
    ...
    def get_datasources_accessible_by_user(  # pylint: disable=invalid-name
        ...
        logger.error("user_perms=%s, schema_perms=%s, user_datasources=%s", user_perms, schema_perms, user_datasources)
        if schema:
