...
class BaseViz:
    ...
    def get_fliters_to_sql(self,query_obj):
        ...
        for i, item in enumerate(filters):
            # sure the col attributr in databases tabel cloum
                ...
                if col_expression.get(col, None) != None:
                ...
                cols_map[col ]= col + " " + op + " " + val_str
                if col in self.datasource.column_names:
                    datasource_filters_map[col] = col + " " + op + " " + val_str
        ...
        if(from_dttm is not None and to_dttm is not None):
            ...
            cols_map[main_dttm_col ]= main_dttm_col + " >= '" + from_dttm.strftime("%Y-%m-%d %H:%M:%S") + "' AND " +  main_dttm_col + " <= '" + to_dttm.strftime("%Y-%m-%d %H:%M:%S") + "'"
            datasource_filters_map[main_dttm_col]  =  main_dttm_col + " >= '" + from_dttm.strftime("%Y-%m-%d %H:%M:%S") + "' AND " +  main_dttm_col + " <= '" + to_dttm.strftime("%Y-%m-%d %H:%M:%S") + "'"
        if filters_sql_str == "":
            filters_sql_str = "1=1"
        return (cols_map,datasource_filters_map)
    ...
    def get_df(self, query_obj: Optional[QueryObjectDict] = None) -> pd.DataFrame:
        ...
        if self.form_data.get("sql").strip():
            fliters_to_map,datasource_filters_map = self.get_fliters_to_sql(query_obj)
            self.query = self.form_data.get("sql").strip()
            # if sql str exsit var @filters,replace it by fliters_to_sql
            import re
            p=re.compile('\$\{(.+?)\}')
            vals =p.findall(self.query)
            logging.info('===============[vals]==========================')
            logging.info('%s'% (vals))
            logging.info('===============[fliters_to_map]==========================')
            logging.info('%s' % (fliters_to_map))
            logging.info('===============[datasource_filters_map]==========================')
            logging.info('%s' % (datasource_filters_map))
            for val in vals :
                v=fliters_to_map.get(val,None)
                if v is not None:
                    self.query =self.query.replace("${"+val+"}",v )
                else:
                    self.query =self.query.replace("${"+val+"}","1=1" )
                    logging.info("The filters do not contains : {0} !".format(val))
            fliters_to_sql =  ' AND '.join(datasource_filters_map.values()) if len(datasource_filters_map.values()) >0 else  '1=1'
            self.query = self.query.replace("@filters", fliters_to_sql)
            logging.info('===============[self.query=]%s==========================', self.query)
    ...
    def get_df_payload(
        ...
        return {
            ...
            "rowcount":  len(df["index"] if isinstance(df, dict) else df.index) if df is not None else 0,
        }
...
class EchartsMultipleLines(NVD3Viz):
    ...
    def get_csv(self) -> Optional[str]:
        ...
        for i, df in enumerate(dfs):
            ...
            df[DTTM_ALIAS] = df[DTTM_ALIAS].dt.tz_localize(None)
            include_index = not isinstance(df.index, pd.RangeIndex)
            df.to_excel(excel_writer = writer,  index=include_index,    sheet_name = "Sheet_" + str(i), **config["CSV_EXPORT"])
        writer.close()
        output.seek(0)
        return output
    ...
    def query_obj(self):
        ...
        q1["groupby"] = self.form_data.get("yaxis1_groupby") or []
        q2["groupby"] = self.form_data.get("yaxis2_groupby") or []
    ...
    def get_df(self, query_obj):
        if not query_obj:
                query_obj = self.query_obj()
        if not query_obj:
            return pd.DataFrame()
        
        query_d1, query_d2 = query_obj.values()
        result1 = self.datasource.query(query_d1)
        result2 = self.datasource.query(query_d2)

        return {"df": (result1.df, result2.df), "index": result1.df.index}


    def to_series(self, df1, df2, dff1, dff2, classed=""):
        dfs = [df1, df2]
       ...
       # line_types = [self.form_data.get("seriestype"), self.form_data.get("seriestype_2")]
        ...
        metrics = [
            {
                "metric": self.form_data.get("metric"),
                "line_type": self.form_data.get("seriestype"),
                "series": dff1.to_dict("series"),
                "groupby_fields":  self.form_data.get("yaxis1_groupby"),
                "df": df1,
                "dff": dff1
            },
            {
                "metric": self.form_data.get("metric_2"),
                "line_type": self.form_data.get("seriestype_2"),
                "series": dff2.to_dict("series"),
                "groupby_fields":  self.form_data.get("yaxis2_groupby"),
                "df": df2,
                "dff": dff2
            },
        ]
        ...
        # types_metric = {}
        # groupby_fields = self.form_data.get("groupby")
        # bar_index = 0
        # # for i,type in enumerate(line_types):
        # #     if type == 'bar' :
        # #         bar_index = i
        # #     types_metric[type] = utils.get_metric_name(metrics[i])
        for i, metric in enumerate(metrics):
            m = metric.get("metric")
            series = metric.get("series")
            groupby_fields = metric.get("groupby_fields")
            line_type = metric.get("line_type")
            df = metric.get("df")
            dff = metric.get("dff")
            m = utils.get_metric_name(m)
            ...
            if not groupby_fields:
                ...
                d = {
                    ...
                    "classed": classed,
                    "values": [
                        {"x": ds, "y": ys[ds] if ds in ys else None} for ds in dff.index
                    ],
                    "yAxis": i + 1,
                    "type": line_type,
                }
                ...
                groupby = df.groupby(groupby_fields)
                for name,group in groupby:
                    group = group.set_index([dff.index.name])
    ...
    def get_data(self, df):
        ...
        groups = fd.get("yaxis1_groupby") or []
        groups_2 = fd.get("yaxis2_groupby") or []
        dff1 = df1.pivot_table(index=DTTM_ALIAS,  values=[metric])
        dff2 = df2.pivot_table(index=DTTM_ALIAS,  values=[metric_2])
        chart_data = self.to_series(df1,df2, dff1, dff2)
        return chart_data
